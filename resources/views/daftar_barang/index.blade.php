...
<div class="container">
    ...
    <form method="GET" action="{{ route('barang.index') }}" class="mb-3 mt-3">
                        ...
                        <tr id="row-{{ $item->id_barang }}" @if($item->kondisi == 'dihapus') class="text-muted" @endif>
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                @if (session('edit_id') == $item->id_barang)
                                    <input form="form-edit-{{ $item->id_barang }}" type="text" name="nama_barang" value="{{ $item->nama_barang }}"
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                @if (session('edit_id') == $item->id_barang)
                                    <input form="form-edit-{{ $item->id_barang }}" type="text" name="jenis_barang" value="{{ $item->jenis_barang }}"
                                ...
                                @if (session('edit_id') == $item->id_barang)
                                    <div>
                                        <img id="preview-{{ $item->id_barang }}"
                                            src="{{ $item->foto_barang ? asset('storage/' . $item->foto_barang) : '#' }}" 
                                            alt="Gambar Barang"
                                            width="100" class="{{ $item->foto_barang ? 'mb-2' : 'd-none' }}"
                                            style="cursor: pointer;" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#photoModal"
                                            data-bs-whatever="{{ asset('storage/' . $item->foto_barang) }}">
                                    </div>
                                    <label for="upload-{{ $item->id_barang }}" class="btn btn-sm btn-primary mt-2">
                                        Ganti Gambar
                                    </label>
                                    <input form="form-edit-{{ $item->id_barang }}" id="upload-{{ $item->id_barang }}" type="file" name="foto_barang" class="d-none"
                                        onchange="previewImage(this, 'preview-{{ $item->id_barang }}')">
                                @else
                                    @if ($item->foto_barang)
                                        <img src="{{ asset('storage/' . $item->foto_barang) }}?{{ time() }}" 
                                            alt="Gambar Barang" width="140"
                                            style="cursor: pointer;" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#photoModal"
                                            data-bs-whatever="{{ asset('storage/' . $item->foto_barang) }}">
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                @if ($item->qr_code_path)
                                    <img src="{{ asset('storage/' . $item->qr_code_path) }}?{{ time() }}" 
                                        alt="QR Code" width="100"
                                        style="cursor: pointer;" 
                                        data-bs-toggle="modal" 
                                        data-bs-target="#qrModal"
                                        data-bs-id="{{ $item->id_barang }}"
                                        data-bs-image="{{ asset('storage/' . $item->qr_code_path) }}?{{ time() }}">
                            ...
                            <td style="align-items: center; vertical-align: middle;" class="text-center">
                                <div class="btn-group-vertical w-100">
                                    @if (session('edit_id') == $item->id_barang)
                                        <form id="form-edit-{{ $item->id_barang }}" action="{{ route('barang.update', $item->id_barang) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn btn-success btn-sm uniform-btn mb-1">Simpan</button>
                                        </form>
                                    ...
                                    @else
                                        <form action="{{ route('barang.edit_mode', $item->id_barang) }}" method="POST" style="display: inline;">
                                            @csrf
                                            <button type="submit" class="btn btn-warning btn-sm uniform-btn mb-1">Edit</button>
                                        </form>
    ...
    <script>
        function previewImage(input, previewId) {
            ...
            if (input.files && input.files[0]) {
                ...
                preview.src = "#";
                // Jangan sembunyikan jika sebelumnya ada gambar, kembalikan ke src lama harusnya.
                // Tapi untuk simplifikasi preview, biarkan logic ini
            }
        }
        ...
        photoModal.addEventListener('show.bs.modal', function (event) {
            const button = event.relatedTarget; 
            const imageUrl = button.getAttribute('data-bs-whatever'); 
            const modalImage = photoModal.querySelector('.modal-body img');
            modalImage.src = imageUrl; 
        });

        // --- Logic Kelola Petugas ---
        ...
        const loadPetugas = () => {
            fetch("{{ route('petugas.index') }}")
                ...
                .catch(() => console.error("Gagal memuat data petugas."));
        };
        ...
        const addPetugasForm = document.getElementById("addPetugasForm");
        if(addPetugasForm){
        ...
        if(petugasModal){
            petugasModal.addEventListener("show.bs.modal", loadPetugas);
        }
    </script> 
    <style>
        .table-responsive {
